...
ProjectZero będzie dla mnie drugą grą wykonaną w Unity 3D (a uwzględniając 2D - trzecią). Ze względu na zakres wykonanych zadań można ją uznać jako pełnoprawny tytuł. To gra której celem nie jest tylko zapewnienie rozrywki ale też poznanie/przypomnienie języków z którymi ma się w obecnych czasach styczność. Warto zaznaczyć, że projekt nie obejmuje wszystkich aspektów każdego z uwzględnionych języków. Dałoby się co prawda to zrobić jednak postawmy sobie pytanie - Czy taka gra nie stałaby się nudna po X jednostek czasu? Moim zdaniem tak. W ProjectZero poruszane są te najistotniejsze elementy jak terminy tablicy, listy, pętli, obiektowego programowania itd.. Wiedza przekazywana jest w taki sposób aby zachęcała graczy do sprawdzenia większej liczby informacji na własną rękę, poza grą. Oprócz tworzenia czegoś co miałoby być użyteczne zależało mi też na tym aby był to projekt, którym warto się pochwalić i zapisać w swoim CV jako udany projekt do kolekcji :) 
